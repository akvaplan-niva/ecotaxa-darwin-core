...
export const occurrenceMapFactory = ({
  ...
  ({
    ...
    const temporary =
      (temporaryMap && temporaryMap.size &&
          /^temporary/.test(object_annotation_hierarchy))
        ? temporaryMap.get(object_annotation_hierarchy) ?? {}
        : {};
    const scientificName = extractScientificName(object_annotation_hierarchy);
    const identificationQualifier = extractIdentificationQualifier(
      object_annotation_hierarchy,
    );
    ...
    const occurrenceStatus = scientificName || temporary.scientificName
      ? "present"
      : undefined;

    const organismQuantity = /(multiple|temporary)/.test(identificationRemarks)
      ? null
      : 1;
    const organismQuantityType =
      /(multiple|temporary)/.test(identificationRemarks)
        ? "multiple"
        : "individuals";
